	...
	MainCode VS_MapOverlay
	{
		...
		[[
			...
			{	
				...
				Out.OriginPos = GH_ToWorldSpace(float3(0.0f, 0.0f, 0.0f), WorldMatrix);
				Out.InstanceIndex = Input.InstanceIndices.y;
				return Out;
			}
		]]
		...
		[[
			...
			PDX_MAIN
			{
				...
				//EK2 - APPLY TERRAIN TEXTURE TO MESH
					...
					Diffuse.a = PdxTex2D( UniqueMap, DIFFUSE_UV_SET ).a;
					//Diffuse.a = 1.0f;
...
RasterizerState OverlayRasterizerState
{
	DepthBias = -5000
...
{
	VertexShader = "VS_MapOverlay"
	PixelShader = "PS_standard"
	BlendState = "alpha_blend"
	DepthStencilState = "depth_no_write"
	RasterizerState = OverlayRasterizerState
	Defines = { "PDX_MESH_SNAP_VERTICES_TO_TERRAIN" "COUNTY_OVERLAY"}
}
